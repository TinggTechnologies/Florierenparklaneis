...
<?php
             ...
             $error = [];
             if(isset($_POST['admin-existing-library-check'])){
                if(isset($_GET['library_id'])){
                    $library_id = $_GET['library_id'];
                   } 
               $library_id = $_POST['library_id'];
               $sql1 = "UPDATE library SET verify='0' WHERE library_id='$library_id'";
               $stmt1 = $conn->prepare($sql1);
               if($stmt1->execute()){
                echo "<script>location.href = 'existing-library.php';</script>";
               }
             }
...
<div class="container mt-5 pt-5">
  ...
  <nav>
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item"><a href="#">Verify Library</a></li>
                </ol>
  </nav>
              ...
              <div class="card info-card sales-card">
                ...
                <div class="card-body">
                  ...
                  <div class="row g-0 text-center">
                    ...
                    <p class="card-text pt-0"><?= $get_data['about']; ?></p>
                    <p class="card-text pt-0"><?= $get_data['date']; ?></p>
                    <hr>
                 
                  <div class="row">
                    <div class="col-sm-12 d-flex align-items-center justify-content-between">
                      <a href="admin-view-library.php?path=uploads/<?= $get_data['pdf']; ?>" class="btn btn-primary form-control w-50">Download</a>
                      <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post">
                      <input type="hidden" name="library_id" value="<?= $library_id; ?>"><br />
                      <button type="submit" name="del-library" class="btn btn-danger form-control mb-4">Delete Book</button>
                    </form>
                    </div>
                  </div>
