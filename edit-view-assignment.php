...
require "backend/edit-assignment.php";
?>
...
<?php
          ...
          if($result->num_rows > 0){
            ...
            $sql = "SELECT * FROM assignment WHERE assignment_id='$library_id' ORDER BY assignment_id DESC";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            if($result->num_rows > 0){
...
<p style="opacity: .8;" class="text-center"><span style="color: green;"><?= $row['lastname']; ?></span>, You can Edit books for your students to have access to.</p>
<form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" enctype="multipart/form-data">

<div class="mb-3 mt-3">
    <select name="course" id="course" class="form-control" >
        <option value="<?php
        if(isset($fetch['subject'])){
         echo $fetch['subject']; 
        }
         ?>"><?php
        if(isset($fetch['subject'])){
...
<option value="<?php
  ...
  <div class="mb-3 mt-3">
    ...
    <input type="file" name="file" id="attachment" class="form-control" value="uploads/<?= $fetch['assignment']; ?>">
    <div class="text-danger">
    <?php
if(isset($error['file'])){
  echo $error['file'];
...
?>
  ...
  <div class="mb-3 mt-3">
    <input name="assignment" id="" rows="7" class="form-control" value=" <?php
      if(isset($fetch['deadline'])){
    echo $fetch['deadline'];
      }
    ?>">
...
?>
  ...
  <button type="submit" class="btn btn-success text-white form-control mb-5" name="library-btn" style="border-radius: 15px;">Edit Assignment</button>
