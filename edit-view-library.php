...
require "backend/edit-library.php";
?>
...
<?php
          ...
          if($result->num_rows > 0){
            ...
            $sql = "SELECT * FROM library WHERE library_id='$library_id' ORDER BY library_id DESC";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            if($result->num_rows > 0){
...
<option value="<?php
  ...
  <div class="mb-3 mt-3">
    ...
    <input type="file" name="file" id="attachment" class="form-control" value="uploads/<?= $fetch['pdf']; ?>">
    <div class="text-danger">
    <?php
if(isset($error['file'])){
  echo $error['file'];
