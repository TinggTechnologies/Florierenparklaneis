...
<nav>
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"><a href="#">Edit Note</a></li>
                </ol>
  </nav>
...
<?php
          ...
          if($result->num_rows > 0){
            ...
            $sql = "SELECT * FROM lesson_note WHERE lesson_note_id='$library_id' ORDER BY lesson_note_id DESC";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
            if($result->num_rows > 0){
...
<p style="opacity: .8;" class="text-center"><span style="color: green;"><?= $row['lastname']; ?></span>, You can Edit Lesson Notes for your students to have access to.</p>
...
<option value="<?php
  ...
  <div class="mb-3 mt-3">
    ...
    <input type="file" name="file" id="attachment" class="form-control" value="uploads/<?= $fetch['lesson_note']; ?>">
    <div class="text-danger">
    <?php
if(isset($error['file'])){
  echo $error['file'];
